...
class SwiftSyllablesHeuristic {
    ...
    class func getSyllablesForWord(_ word: String) -> Int {
        ...
        let last2CharsSubstring: String? = substringAtIndex(baseWord, index: -2)
        let last3CharsSubstring: String? = substringAtIndex(baseWord, index: -3)

        if let last2CharsSubstring = substringAtIndex(baseWord, index: -2) {
        ...
        let first2Substring = substringAtIndex(baseWord, index: 2)
        ...
        guard let first2CharsIndex = rangeForIndex(baseWord, index: 2) else { return 1 }
        guard let first2CharsSubstring = substringAtIndex(baseWord, index: 2) else { return 1 }
        let first3CharsIndex = baseWord.index(first2CharsIndex, offsetBy: 1)
        if first2CharsSubstring == "bi" || baseWord.substring(to: first3CharsIndex) == "tri" {
        ...
        if first2CharsSubstring == "co" && isVowel(baseWord[first3CharsIndex]) {
            if let first6CharsSubstring = substringAtIndex(baseWord, index: 6) {
